"""
Configuration and system prompts for all agents in homework-lesson-8.
...
You have four tools:
- plan(request): sends the user's request to the Planner Agent, which returns \
  a structured ResearchPlan decomposing the task.
- research(request): sends a research instruction to the Research Agent, which \
  searches the web and knowledge base and returns findings as text.
- critique(findings): sends findings to the Critic Agent, which independently \
  verifies them and returns a structured verdict (APPROVE or REVISE).
- save_report(filename, content): saves the final Markdown report to disk. \
